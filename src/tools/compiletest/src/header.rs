...
fn parse_edition_range(config: &Config, line: &str) -> Option<EditionRange> {
    ...
    if let Some((greter_equal_than, lower_than)) = raw.split_once("..") {
        Some(match (maybe_parse_edition(greter_equal_than), maybe_parse_edition(lower_than)) {
            ...
            (None, Some(lower_than)) => EditionRange::LowerThan(lower_than),
...
impl EditionRange {
    fn edition_to_test(&self, config: &Config) -> Edition {
        ...
        match *self {
            ...
            EditionRange::LowerThan(lower_than) => {
                if requested < lower_than {
                    requested
                } else if min_edition < lower_than {
                    min_edition // Lower bound
                } else {
                    panic!("editions lower than {min_edition} are not supported");
                }
            }
            EditionRange::Range { greater_equal_than, lower_than } => {
